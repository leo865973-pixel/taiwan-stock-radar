...
def fetch_ma60(code: str) -> tuple[float | None, float | None]:
    """
    透過 FinMind API 獲取個股過去歷史資料，並計算 60 日均線 (MA60) 與建議停損價。
    回傳 (ma60, suggested_stop_loss)，若失敗回傳 (None, None)。
    """
    try:
        import datetime
        from dateutil.relativedelta import relativedelta
        start_date = (datetime.date.today() - relativedelta(days=120)).strftime("%Y-%m-%d")
        ...
        if data.get("msg") != "success":
            logger.warning(f"FinMind MA60 [{code}] 錯誤: {data.get('msg')}")
            return None, None
        ...
        closes = prices[-60:]
        ma60 = float(round(sum(closes) / len(closes), 2))
        stop_loss = round(ma60 * 0.97, 2)
        return ma60, stop_loss
        
    except Exception as e:
        logger.warning(f"FinMind MA60 查詢失敗 [{code}]: {e}")
        return None, None
...
def calc_health_score_and_advice(s: dict) -> dict:
    ...
    s 必須包含: price, ma60, inst_net_buy_3d, it_continuous_buy, revenue_double_growth
    """
    price = s.get("price")
    ma60 = s.get("ma60")
    ...
    if price and ma60:
        if price >= ma60 * 1.05:
            score += 20
            tech_status = "股價穩站月線之上，趨勢強勢。"
        elif price >= ma60:
            score += 10
            tech_status = "股價維持在月線之上，算是安全。"
        elif price >= ma60 * 0.97:
            score -= 10
            tech_status = "股價已經跌破月線，正在防守邊緣測試。"
        else:
            score -= 20
            tech_status = "股價明顯跌破防守線，趨勢轉弱。"
    ...
    if score >= 75:
        ...
        light = "🔴 破線警報"
        advice = f"【危險警告】{tech_status}{chip_status} 如果手上有這檔，請嚴格執行停損，不要抱著不放。"
